  ...
  handler: async (message) => {
    ...
    for (const tenantRepository of tenantRepositories) {
      if (tenantRepository.sourceControl === "gitlab") {
        console.log(
          "Warn: Transform for gitlab repos is not yet implemented; external_id =",
          tenantRepository.externalId,
        );
        continue;
      }

      promises.push(
        transformRepositoryEvent.publish(
          {
...
export const cronHandler = async () => {
  ...
  const tenantTransformInput = cronEnabledTenants.map((tenant) => ({
    dbUrl: tenant.dbUrl,
...
export const apiHandler = ApiHandler(async (ev) => {
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({
      message: `Transforming tenant "${tenant.name}" in period (${from.toISOString()}...${to.toISOString()})`,
    }),
  };
});
