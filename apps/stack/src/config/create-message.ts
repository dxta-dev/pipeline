...
type Content<Shape extends ZodRawShape> = z.infer<
  ZodObject<Shape, "strip", ZodAny>
>;
    ...
    await sqs.send(
      new SendMessageCommand({
        ...
        MessageBody: JSON.stringify(
          messageSchema.parse({ content, metadata, kind }),
        ),
      }),
    );
  };

  const sendAll: BatchSend<Shape, MetadataShape> = async (
    contentArray,
    metadata,
  ) => {
    ...
    for (let i = 0; i < contentArray.length; i += 10) {
      ...
      const Entries = contentBatch
        .map((content) =>
          JSON.stringify(messageSchema.parse({ content, metadata, kind })),
        )
        .map((MessageBody) => ({
          Id: nanoid(),
          MessageBody,
...
export function QueueHandler(
  ...
  crawlNamespaceMap: Map<string, EventNamespaceType>,
) {
  return async (event: SQSEvent) => {
    if (event.Records.length > 1)
      console.warn(
        "WARNING: QueueHandler should process 1 message but got",
        event.Records.length,
      );
    for (const record of event.Records) {
      const parsedEvent = JSON.parse(record.body) as unknown as MessagePayload<
        ZodRawShape,
        ZodRawShape
      >;

      // Enforces all queue handlers to have dbUrl since it's required for extracts and crawls
      const dbUrl = parsedEvent.metadata?.dbUrl as unknown as
        | Tenant["dbUrl"]
        | undefined;
      if (dbUrl === undefined) {
      ...
      const { sender, handler } = (map as MessageKindMap).get(
        parsedEvent.kind,
      ) ?? { sender: null, handler: null };
      ...
      const crawlId = parsedEvent.metadata?.crawlId as unknown as
        | number
        | undefined;
      ...
      if (!validatedMessage.success) {
        ...
        await crawlFailed(
          ...
          crawlId,
          crawlEventNamespace,
          `Error: Failed to parse message ${parsedEvent.kind} for crawl id: ${crawlId} - ${crawlEventNamespace}`,
        );
        return;
      }
      ...
      if (!handlerError) {
        console.log(
          "Handled message",
          createLog(validatedMessage.data, parsedEvent.kind, propertiesToLog),
        );
        try {
...
export type MessageKindMap = Map<
  string,
  ReturnType<typeof createMessageHandler>
>;

type CreateMessageHandlerProps<
  Shape extends ZodRawShape,
  MetadataShape extends ZodRawShape,
> = {
  contentShape: Shape;
