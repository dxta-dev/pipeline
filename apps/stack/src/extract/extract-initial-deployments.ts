...
import {
  deploymentsSenderHandler,
  extractDeploymentsPage,
} from "./extract-deployments";
...
export const apiHandler = ApiHandler(async (ev) => {
  ...
  if (!lambdaContextValidation.success) {
    console.log(
      ...
      lambdaContextValidation.error.issues,
    ); // TODO: compliance check, might be insufficient_scope or something
    return {
      statusCode: 401,
  ...
  const superDb = drizzle(
    createClient({
      url: Config.SUPER_DATABASE_URL,
      authToken: Config.SUPER_DATABASE_AUTH_TOKEN,
    }),
  );
  ...
  const target = await db
    ...
    .innerJoin(
      repositories,
      eq(repositories.externalId, deploymentEnvironments.repositoryExternalId),
    )
  ...
  if (!target)
    return {
      statusCode: 404,
      body: JSON.stringify({
        message: `Invalid deployment id ${deploymentId} for tenant ${tenant.name}`,
      }),
    };
  ...
  let instanceId = -1;
  try {
    const crawl = await setInstance(
      {
        repositoryId: target.repository.id,
        userId: sub,
        since: nullDurationDateAt,
        until: nullDurationDateAt,
      },
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({
        message: `Failed to create deployment crawl instance for repository ${target.namespace.name}/${target.repository.name}`,
      }),
    };
  }
  ...
  try {
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({
        message: `Failed to retrieve first page #${firstPage} of deployments for repository ${target.namespace.name}/${target.repository.name}`,
      }),
    };
  }

  const totalPages = lastPage || firstPageResult.pagination.totalPages;
  const arrayOfExtractDeploymentsPageMessageContent: Parameters<
    typeof deploymentsSenderHandler.sender.send
  >[0][] = [];
  ...
  if (arrayOfExtractDeploymentsPageMessageContent.length === 0) {
    return {
      statusCode: 200,
      body: JSON.stringify({
        message: `Extracted ${firstPageResult.deployments.length} deployments for tenant ${tenant.name} repository ${target.namespace.name}/${target.repository.name}`,
      }),
    };
  }
  ...
  try {
    ...
    return {
      statusCode: 500,
      body: JSON.stringify({
        message: `Failed to enqueue pages ${firstPage + 1}..${totalPages} for tenant ${tenant.name} repository ${target.namespace.name}/${target.repository.name}`,
      }),
    };
  }
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({
      message: `Extracted ${firstPageResult.deployments.length} deployments and enqueued pages ${firstPage + 1}..${totalPages} for tenant ${tenant.name} repository ${target.namespace.name}/${target.repository.name}`,
    }),
  };
});
