...
type ExtractDefaultBranchCommitsContext = Context<
  GetCommitsSourceControl,
  GetCommitsEntities
>;
...
export const eventHandler = EventHandler(
  ...
  async (ev) => {
    ...
    const arrayOfExtractDefaultBranchCommitsPageMessageContent: Parameters<
      typeof commitsSenderHandler.sender.send
    >[0][] = [];
    ...
    await insertEvent(
      {
        crawlId,
        eventNamespace: "defaultBranchCommit",
