...
type ExtractWorkflowDeploymentStatusContext = Context<
  GetWorkflowDeploymentStatusSourceControl,
  GetWorkflowDeploymentStatusEntities
>;
...
export const workflowDeploymentStatusSenderHandler = createMessageHandler({
  ...
  handler: async (message) => {
    ...
    if (deployment.status !== null && deployment.status !== "pending") {
      console.log(
        "Skipping extract-workflow-deployment-status for status:",
        deployment.status,
      );
      return;
    }
...
export const eventHandler = EventHandler(
  ...
  async (ev) => {
    ...
    const arrayOfExtractDeploymentStatusMessageContent: Parameters<
      typeof workflowDeploymentStatusSenderHandler.sender.send
