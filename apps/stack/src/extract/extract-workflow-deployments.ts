...
type ExtractWorkflowDeploymentsContext = Context<
  GetWorkflowDeploymentsSourceControl,
  GetWorkflowDeploymentsEntities
>;
...
export const workflowDeploymentsSenderHandler = createMessageHandler({
  ...
  handler: async (message) => {
    ...
    const { namespace, repository, workflowId, branch, perPage, page } =
      message.content;
...
export const eventHandler = EventHandler(
  ...
  async (ev) => {
    ...
    if (workflows.length == 0) {
      console.log(
        "No deployment workflows defined for repository",
    ...
    const workflowDeploymentsFirstPages = await Promise.all(
      workflows.map((workflow) =>
        getWorkflowDeployments(
          ...
          { ...staticContext, ...dynamicContext },
        ).then((result) => ({ workflow, result })),
      ),
    );

    const arrayOfExtractDeploymentsPageMessageContent: Parameters<
      typeof workflowDeploymentsSenderHandler.sender.send
    >[0][] = [];
