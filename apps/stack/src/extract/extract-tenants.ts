...
export const tenantSenderHandler = createMessageHandler({
  ...
  handler: async (message) => {
    ...
    if (repos.length === 0) {
      console.log(
        `Warn: no repositories to extract for tenant: ${message.content.tenantDomain}`,
      );
      return;
    }
...
export const cronHandler = async () => {
  ...
  const tenantCrawlInput = cronEnabledTenants.map((tenant) => ({
    dbUrl: tenant.dbUrl,
    crawlUserId: tenant.crawlUserId,
...
export const apiHandler = ApiHandler(async (ev) => {
  ...
  if (from.getTime() > to.getTime()) {
    return {
      statusCode: 400,
      body: JSON.stringify({
        error: `the lower bound of the period since: ${from.toISOString()} is greater than the upper bound until: ${to.toISOString()}`,
      }),
    };
  }
  ...
  await sender.sendAll(
    [{ dbUrl: tenant.dbUrl, crawlUserId: sub, tenantDomain: tenant.name }],
    {
      version: -1,
  ...
  return {
    statusCode: 200,
    body: JSON.stringify({
      message: `Extracting tenant ${tenant.name} repositories in period (${from.toISOString()}...${to.toISOString()})`,
    }),
  };
});
