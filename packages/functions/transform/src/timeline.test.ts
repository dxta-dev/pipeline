...
const fixtures = [
  ...
  string,
  {
    pr: typeof pr1 | typeof pr5 | typeof pr143;
    expected: typeof pr1Expected | typeof pr5Expected | typeof pr143Expected;
  },
][];
...
describe("timelines", () => {
  describe.each(fixtures)("%s", (_name, { pr, expected }) => {
    ...
    const result = calculateTimeline(
      keys as unknown as TimelineMapKey[],
      map as Map<TimelineMapKey, MergeRequestNoteData | TimelineEventData>,
      { authorExternalId, createdAt: new Date(pr.openedAt) },
    );

    const { startedCodingAt, mergedAt, reviewed, reviewDepth, handover } =
      result;

    const getTime = (date: Date | null) => date?.getTime() || null;

    test("startedCodingAt", () => {
      expect(getTime(startedCodingAt)).toEqual(
        getTime(expected.startedCodingAt),
      );
    });
