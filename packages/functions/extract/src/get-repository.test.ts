...
describe("get-repository", () => {
  describe("getRepository", () => {
    test("should insert values into db", async () => {
      const { namespace, repository } = await getRepository(
        { externalRepositoryId: 1000, namespaceName: "", repositoryName: "" },
        context,
      );
      ...
      const namespaceRow = await db
        .select()
        .from(namespaces)
        .where(eq(namespaces.externalId, namespace.externalId))
        .get();
      ...
      const repositoryRow = await db
        .select()
        .from(repositories)
        .where(eq(repositories.externalId, repository.externalId))
        .get();
