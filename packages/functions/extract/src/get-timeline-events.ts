...
export type GetTimelineEventsSourceControl = Pick<
  SourceControl,
  "fetchTimelineEvents"
>;
export type GetTimelineEventsEntities = Pick<
  Entities,
  | "namespaces"
  | "repositories"
  | "mergeRequests"
...
export type GetTimelineEventsFunction = ExtractFunction<
  GetTimelineEventsInputs,
...
export const getTimelineEvents: GetTimelineEventsFunction = async (
  ...
  const { timelineEvents } =
    await integrations.sourceControl.fetchTimelineEvents(
      repository,
      namespace,
      mergeRequest,
    );

  const nonCommitEvents = timelineEvents.filter(
  ...
  const uniqueTimelineActors = [
    ...nonCommitEvents
      .reduce(
        (externalIdToActor, event) =>
          event.actorId
            ? externalIdToActor.set(event.actorId, {
                // actorId is optional due to commit events
            ...
            : externalIdToActor,
        new Map<number, NewMember>(),
      )
      .values(),
  ];

  const insertedUniqueTimelineActors =
    uniqueTimelineActors.length === 0
      ? []
      : await db.transaction(async (tx) => {
          return Promise.all(
            uniqueTimelineActors.map((actor) =>
              tx
  ...
  if (insertedUniqueTimelineActors.length > 0) {
    await db
      ...
      .values(
        insertedUniqueTimelineActors.map((member) => ({
          memberId: member.id,
          repositoryId,
        })),
      )
  ...
  const insertedTimelineEvents = await db.transaction(async (tx) => {
    return Promise.all(
      timelineEvents.map((event) =>
        tx
          .insert(entities.timelineEvents)
          .values(event)
          .onConflictDoUpdate({
            target: [
              entities.timelineEvents.externalId,
              entities.timelineEvents.mergeRequestId,
              entities.timelineEvents.type,
            ],
            set: {
