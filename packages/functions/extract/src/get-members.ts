...
export type GetMembersEntities = Pick<
  Entities,
  "members" | "repositoriesToMembers"
>;

export type GetMembersFunction = ExtractFunction<
  GetMembersInput,
  GetMembersOutput,
...
export const getMembers: GetMembersFunction = async (
  ...
  const { members, pagination } = await integrations.sourceControl.fetchMembers(
    externalRepositoryId,
    namespaceName,
    repositoryName,
    perPage,
