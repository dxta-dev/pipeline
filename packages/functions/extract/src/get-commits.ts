...
export type GetCommitsEntities = Pick<
  Entities,
  "commits" | "repositoryShaTrees" | "repositoryShas"
>;

export type GetCommitsFunction = ExtractFunction<
  GetCommitsInput,
  GetCommitsOutput,
  GetCommitsSourceControl,
  GetCommitsEntities
>;
...
export const getCommits: GetCommitsFunction = async (
  ...
  const { commits: commitData, pagination } =
    await integrations.sourceControl.fetchCommits(
      repository,
      namespace,
      perPage,
      ref,
  ...
  if (commitData.length === 0 && pagination.totalPages === 1)
    return {
      commits: [],
      paginationInfo: pagination,
    };
  ...
  const uniqueCommitShas = Array.from(
    new Set([...commitData.map((x) => [x.id, ...x.parents]).flat()]).values(),
  );
    ...
    return Promise.all(
      uniqueCommitShas.map((commitSha) =>
        tx
          ...
          .values({ repositoryId: repository.id, sha: commitSha })
          .onConflictDoUpdate({
            target: [
  ...
  const shaTreeNodeList = commitData
    .map((data) =>
      data.parents.map(
        (parent) =>
          ({
          ...
          }) satisfies NewShaTreeNode,
      ),
    )
    .flat();

  await db
    .insert(entities.repositoryShaTrees)
    .values(shaTreeNodeList)
    .onConflictDoNothing()
    .run();

  const newCommits: NewCommit[] = commitData.map(
    (x) =>
      ({
