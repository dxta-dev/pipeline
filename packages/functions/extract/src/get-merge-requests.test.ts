    ...
    (
      ...
      if (timePeriod) {
        filteredArray = mergeRequestArray.filter((mergeRequest) => {
          return (
            mergeRequest.createdAt > timePeriod.from &&
            mergeRequest.createdAt < timePeriod.to
          );
        });
      } else {
  ...
  describe("getMergeRequests", () => {
    test("should create insert merge request data in the database", async () => {
      ...
      const mergeRequestData = await db
        .select()
        .from(context.entities.mergeRequests)
        .all();
      expect(mergeRequestData.length).toEqual(mergeRequests.length);

      for (const mergeRequest of mergeRequestData) {
        expect(
          mergeRequests.find((mr) => mr.externalId === mergeRequest.externalId),
        ).toBeDefined();
        expect(
          mergeRequests.find((mr) => mr.id === mergeRequest.id),
        ).toBeDefined();
      }
    });
