...
beforeAll(async () => {
  ...
  fetchWorkflowDeployments = jest.fn(
    (
      repository,
      _namespace,
      workflowId,
...
describe("get-workflow-deployments:", () => {
  describe("getWorkflowDeployments", () => {
    test("should insert workflow deployment data in the database", async () => {
      ...
      const workflowDeploymentRows = await db
        .select()
      ...
      for (const workflowDeploymentRow of workflowDeploymentRows) {
        const rowMatchedWorkflowDeployment = workflowDeploymentRows.find(
          (ci) => ci.externalId === workflowDeploymentRow.externalId,
        );
        expect(rowMatchedWorkflowDeployment).toBeDefined();
        if (!rowMatchedWorkflowDeployment) return;
        expect(rowMatchedWorkflowDeployment.id).toEqual(
          workflowDeploymentRow.id,
        );
      }
    });
  });
