...
beforeAll(async () => {
  ...
  fetchNamespaceMembers = jest.fn(
    (_externalNamespaceId, namespaceName, perPage: number, page?: number) => {
  ...
  describe("getNamespaceMembers", () => {
    test("should insert member data in the database", async () => {
      ...
      for (const memberRow of memberRows) {
        const rowMatchedMember = members.find(
          (m) => m.externalId === memberRow.externalId,
        );
