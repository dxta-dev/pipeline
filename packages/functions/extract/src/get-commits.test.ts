...
describe("get-commits:", () => {
  describe("getCommits", () => {
    test("should insert commit data in the database", async () => {
      ...
      const shaTreesRows = await db
        .select()
        .from(context.entities.repositoryShaTrees)
        .all();
      const shaRows = await db
        .select()
        .from(context.entities.repositoryShas)
        .all();
      ...
      for (const commitRow of commitRows) {
        const rowMatchedCommit = commits.find(
          (commit) => commit.repositoryShaId === commitRow.repositoryShaId,
        );
