...
let fetchTimelineEvents: jest.MockedFunction<
  GetTimelineEventsSourceControl["fetchTimelineEvents"]
>;
...
beforeAll(async () => {
  ...
  fetchTimelineEvents = jest.fn(
    (
      repository: Repository,
      namespace: Namespace,
      mergeRequest: MergeRequest,
    ): ReturnType<GetTimelineEventsSourceControl["fetchTimelineEvents"]> => {
            ...
            timelineEvents: [
              {
                ...
                data: {
                  ...
                  committedDate: "2023-01-02T00:00:00.000Z",
                },
              },
              {
                externalId: 54321,
...
describe("get-timeline-events:", () => {
  describe("getTimelineEvents", () => {
    test("should insert timeline event data into db", async () => {
      ...
      const timelineEventsRows = await db
        .select()
        .from(context.entities.timelineEvents)
        .all();
      expect(timelineEventsRows).toHaveLength(2);

      for (const timelineEvent of timelineEventsRows) {
        expect(
          timelineEvents.find((te) => te.id === timelineEvent.id),
        ).toBeDefined();
        expect(
          timelineEvents.find(
            (te) => te.externalId === timelineEvent.externalId,
          ),
        ).toBeDefined();
        expect(
          timelineEvents.find(
            (te) => te.mergeRequestId === timelineEvent.mergeRequestId,
          ),
        ).toBeDefined();
        expect(
          timelineEvents.find((te) => te.type === timelineEvent.type),
        ).toBeDefined();
      }
    });
  });
