...
export type GetMergeRequestNotesSourceControl = Pick<
  SourceControl,
  "fetchMergeRequestNotes"
>;
export type GetMergeRequestNotesEntities = Pick<
  Entities,
  | "namespaces"
  | "repositories"
  | "mergeRequests"
...
export type GetMergeRequestNotesFunction = ExtractFunction<
  GetMergeRequestNotesInput,
  GetMergeRequestNotesOutput,
  GetMergeRequestNotesSourceControl,
  GetMergeRequestNotesEntities
>;
...
export const getMergeRequestNotes: GetMergeRequestNotesFunction = async (
  ...
  const { mergeRequestNotes } =
    await integrations.sourceControl.fetchMergeRequestNotes(
      repository,
      namespace,
      mergeRequest,
    );

  const mergeRequestNoteUniqueAuthors = [
    ...mergeRequestNotes
      .reduce(
        (externalIdToAuthor, note) =>
  ...
  const insertedMembers = await db.transaction(async (tx) => {
    return Promise.all(
      mergeRequestNoteUniqueAuthors.map((author) =>
        tx
  ...
  const insertedMergeRequestNotes = await db.transaction(async (tx) =>
    Promise.all(
      mergeRequestNotes.map((mergeRequestNote) =>
        tx
          .insert(entities.mergeRequestNotes)
          .values(mergeRequestNote)
          .onConflictDoUpdate({
            target: [
              entities.mergeRequestNotes.mergeRequestId,
              entities.mergeRequestNotes.externalId,
            ],
            set: {
