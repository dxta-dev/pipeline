...
let context: Context<
  ...
  GetMergeRequestNotesEntities
>;
let fetchMergeRequestNotes: jest.MockedFunction<
  GetMergeRequestNotesSourceControl["fetchMergeRequestNotes"]
>;
...
beforeAll(async () => {
  ...
  fetchMergeRequestNotes = jest.fn(
    (
...
describe("get-merge-request-notes:", () => {
  describe("getMergeRequestNotes", () => {
    test("should insert merge request note data in the db", async () => {
      ...
      const mergeRequesNoteRows = await db
        .select()
        .from(context.entities.mergeRequestNotes)
        .all();
      expect(mergeRequesNoteRows).toHaveLength(3);

      for (const mergeRequestNoteRow of mergeRequesNoteRows) {
        const extractedMergeRequestNote = mergeRequestNotes.find(
          (mergeRequestNote) =>
      ...
      for (const memberRow of memberRows) {
        const extractedMember = members.find(
          (member) => member.externalId === memberRow.externalId,
        );
