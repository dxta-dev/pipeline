...
export const mergeRequestMetrics = sqliteTable(
  "merge_request_metrics",
  {
    id: integer("id").primaryKey(),

    usersJunk: integer("users_junk")
      .notNull()
      .references(() => mergeRequestUsersJunk.id),

    repository: integer("repository")
      .notNull()
      .references(() => repositories.id),

    mergeRequest: integer("merge_request")
      .notNull()
      .references(() => mergeRequests.id),

    datesJunk: integer("dates_junk")
      .notNull()
      .references(() => mergeRequestDatesJunk.id),
  ...
  (metrics) => ({
    usersJunkIndex: index("merge_request_metrics_users_junk_idx").on(
      metrics.usersJunk,
    ),
    datesJunkIndex: index("merge_request_metrics_dates_junk_idx").on(
      metrics.datesJunk,
    ),
    repositoryIndex: index("merge_request_metrics_repository_idx").on(
      metrics.repository,
    ),
    mergeRequestIndex: index("merge_request_metrics_merge_request_idx").on(
...
export type NewMergeRequestMetric = InferInsertModel<
  typeof mergeRequestMetrics
>;
