...
export const mergeRequestEvents = sqliteTable(
  "merge_request_events",
  {
    ...
    subject: integer("subject")
      .notNull()
      .references(() => forgeUsers.id),
    occuredOn: integer("occured_on")
      .notNull()
      .references(() => dates.id),
    commitedAt: integer("commited_at")
      .notNull()
      .references(() => dates.id),
    ...
    mergeRequestEventType: Enum("merge_request_event_type", {
      enum: MergeRequestEventTypes,
    }).notNull(),
  ...
  (events) => ({
    eventsOccuredOnIndex: index("merge_request_events_occured_on_idx").on(
      events.occuredOn,
    ),
    eventsMergeRequestIndex: index("merge_request_events_merge_request_idx").on(
      events.mergeRequest,
    ),
    eventsCommitedAtIndex: index("merge_request_events_commited_at_idx").on(
      events.commitedAt,
    ),
  }),
);
