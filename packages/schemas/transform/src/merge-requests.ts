...
export const mergeRequests = sqliteTable('merge_requests', {
  ...
  targetBranch: integer('target_branch').notNull().references(() => branches.id).default(sql`1`),
  sourceBranch: integer('source_branch').notNull().references(() => branches.id).default(sql`1`),
