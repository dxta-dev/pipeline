...
export class GitlabSourceControl implements SourceControl {
  ...
  async fetchUserInfo(
    ...
    const result = await this.api.Users.show(externalId, {
      showExpanded: true,
    });
  ...
  async fetchNamespaceMembers(
    ...
    const { data, paginationInfo } = await this.api.GroupMembers.all(
      externalNamespaceId,
      {
        perPage,
        page: page || 1,
  ...
  async fetchMembers(
    ...
    const { data, paginationInfo } = await this.api.ProjectMembers.all(
        ...
        (mr) =>
          ({
            ...
            closedAt: mr.closed_at
              ? new Date(mr.closed_at as unknown as string)
              : undefined,
  ...
  async fetchMergeRequestDiffs(
    ...
    const { data, paginationInfo } = (await this.api.MergeRequests.allDiffs(
      ...
      mergeRequest.canonId,
      {
        ...
        perPage,
        pagination: "offset",
      } as Sudo & ShowExpanded<true>,
    )) as unknown as {
      data: MergeRequestDiffSchema[];
  ...
  async fetchMergeRequestNotes(
    ...
    const { data } = await this.api.MergeRequestNotes.all(
      repository.externalId,
      mergeRequest.canonId,
      {
        showExpanded: true,
