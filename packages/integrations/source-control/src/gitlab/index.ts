...
import type { NewRepository, NewNamespace, NewMergeRequestWithMergeCommitSha, NewMember, NewMergeRequestDiff, Repository, Namespace, MergeRequest, NewMergeRequestCommit, NewMergeRequestNote, NewTimelineEvents, NewCicdWorkflow, NewCicdRun, NewDeploymentWithSha, Deployment } from "@dxta/extract-schema";
...
export class GitlabSourceControl implements SourceControl {
  ...
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  async fetchMergeRequests(externalRepositoryId: number, namespaceName = '', repositoryName = '', repositoryId: number, perPage: number, creationPeriod?: TimePeriod, page?: number): Promise<{ mergeRequests: NewMergeRequestWithMergeCommitSha[], pagination: Pagination }> {
