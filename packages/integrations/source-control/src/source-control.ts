import type { NewRepository, NewNamespace, NewMergeRequestWithMergeCommitSha, NewMember, NewMergeRequestDiff, NewMergeRequestCommit, NewMergeRequestNote, NewTimelineEvents, NewCicdWorkflow, NewCicdRun, NewCommit, NewDeploymentWithSha, Deployment } from "@dxta/extract-schema";
...
export interface SourceControl {
  ...
  fetchMergeRequests(externalRepositoryId: number, namespaceName: string, repositoryName: string, repositoryId: number, perPage: number, creationPeriod?: TimePeriod, page?: number, totalPages?: number): Promise<{ mergeRequests: NewMergeRequestWithMergeCommitSha[], pagination: Pagination }>;
