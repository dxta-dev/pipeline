...
const FILE_STATUS_FLAGS_MAPPING: Record<
  ...
  | "changed"
  | "unchanged",
  Pick<NewMergeRequestDiff, "newFile" | "renamedFile" | "deletedFile">
> = {
  modified: {
    newFile: false,
    renamedFile: false,
...
function mapWorkflowRunStatus(
...
): (typeof deploymentsStatusEnum)[number] | null {
  ...
  switch (status) {
    ...
    case "failure":
      console.log(
        new Error(
          "GithubSourceControl@mapWorkflowRunStatus error: Unexpected status 'failure'",
        ),
      );
      return "failure";
    case "neutral":
      console.log(
        new Error(
          "GithubSourceControl@mapWorkflowRunStatus error: Unexpected status 'neutral'",
        ),
      );
      return "unknown";
    case "skipped":
    ...
    case "stale":
      console.log(
        new Error(
          "GithubSourceControl@mapWorkflowRunStatus warn: Unexpected status 'stale'",
        ),
      );
      return "unknown";
    case "success":
      console.log(
        new Error(
          "GithubSourceControl@mapWorkflowRunStatus error: Unexpected status 'success'",
        ),
      );
      return "success";
    case "timed_out":
    ...
    default:
      console.log(
        new Error(
          `GithubSourceControl@mapWorkflowRunStatus error: Really unexpected status '${status}'`,
        ),
      );
      return "unknown";
  }
...
function mapWorkflowRunConclusion(
  ...
  switch (v) {
    ...
    default:
      console.log(
        new Error(`GithubSourceControl@mapWorkflowRunConclusion error:`),
      );
      return "unknown";
  }
}
...
const dateQueryStrings = {
  dateTimeRange: (since: Date, until: Date) =>
    `${since.toISOString().slice(0, 19)}+00:00..${until.toISOString().slice(0, 19)}+00:00`,
...
export class GitHubSourceControl implements SourceControl {
  ...
  async fetchNamespaceMembers(
    _externalNamespaceId: number,
    namespaceName: string,
    perPage: number,
    page?: number,
  ): Promise<{ members: NewMember[]; pagination: Pagination }> {
    page = page || 1;
  ...
  async fetchMembers(
    ...
    try {
      ...
      if (
        ...
        console.log(
          "SUPPRESSED: Must have push access to view repository collaborators.",
        );
        return {
          members: [],
          pagination: {
  ...
  async fetchMergeRequestDiffs(
    ...
    mergeRequestDiffs: NewMergeRequestDiff[];
    pagination: Pagination;
  }> {
    page = page || 1;
    ...
    return {
      mergeRequestDiffs: result.data.map((mergeRequestFile) => ({
        ...
        oldPath:
          mergeRequestFile.previous_filename || mergeRequestFile.filename,
        aMode: "",
        bMode: "",
  ...
  async fetchMergeRequestCommits(
    ...
    namespace: Namespace,
    mergeRequest: MergeRequest,
  ): Promise<{ mergeRequestCommits: NewMergeRequestCommit[] }> {
    ...
    return {
      mergeRequestCommits: response.data.map(
        (mrc) =>
          ({
            mergeRequestId: mergeRequest.id,
            externalId: mrc.sha,
  ...
  async fetchMergeRequestNotes(
    ...
    namespace: Namespace,
    mergeRequest: MergeRequest,
  ): Promise<{ mergeRequestNotes: NewMergeRequestNote[] }> {
    ...
    return {
      mergeRequestNotes: response.data.map(
        (mergeRequestNote) =>
          ({
  ...
  async fetchTimelineEvents(
    ...
    namespace: Namespace,
    mergeRequest: MergeRequest,
  ): Promise<{ timelineEvents: NewTimelineEvents[] }> {
  ...
  async fetchCommits(
    ...
    period?: TimePeriod,
    page?: number,
  ): Promise<{ commits: CommitData[]; pagination: Pagination }> {
    page = page || 1;
  ...
  async fetchWorkflowDeployments(
    repository: Repository,
    namespace: Namespace,
    workflowId: number,
    timePeriod: TimePeriod,
    ...
    branch?: string,
    page?: number,
  ): Promise<{ deployments: NewDeploymentWithSha[]; pagination: Pagination }> {
    page = page || 1;
    ...
    const cicdRuns = response.data.workflow_runs.map(
      (run) =>
        ({
          externalId: run.id,
          ...
          deployedAt:
            mapWorkflowRunStatus(run.status, run.conclusion) === "success"
  ...
  async fetchDeployments(
    ...
    const deployments = response.data.map(
      (deployment) =>
        ({
  ...
  async fetchDeployment(
    ...
    const firstFailureStatus = orderedData.find(
    ...
    const lastUpdatedAt = finalStatus
      ? new Date(finalStatus.updated_at)
      : deployment.updatedAt;
    const hasThirtyDaysPassedSinceDeploymentStart =
      deployment.createdAt.getTime() + 30 * 24 * 60 * 60 * 1000 >
      lastUpdatedAt.getTime();

    const isPending =
      !finalStatus ||
      finalStatus.state === "in_progress" ||
      finalStatus.state === "queued" ||
      finalStatus.state === "pending";
